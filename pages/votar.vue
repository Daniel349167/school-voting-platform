...
type Candidato = { id: number; nombre: string; descripcion?: string }
...
const verificarDNI = async () => {
  ...
  const res = await $fetch('/api/votos/verificar', {
    method:'POST',
    body:{ dni: dni.value.trim(), grado: grado.value, seccion: seccion.value }
...
<style scoped>
/* Fondo: azul/índigo que combina con el card */
.vote-bg{
  min-height:100vh;
...
/* Card un poco más grande */
.vote-shell{
  width:min(1650px, 98%); /* card más ancho */
  margin-top: 90px;
}
/* Cuando estamos en el paso "votar", sube el card completo */
.vote-shell.votar{
  margin-top: 38px;            /* súbelo: ajusta a -40/-60 según prefieras */
  transition: margin-top .2s;   /* animación sutil */
}
...
.vote-card{
  ...
  overflow:hidden;
  min-height: 380px; /* card más alto */
  display: flex;
  flex-direction: column; 
}


/* Hero con onda (teal->blue, más limpio) */
.hero{position:relative;background:linear-gradient(90deg,#167b77,#267bbb);padding:18px 22px 54px;color:#fff}
.hero .hero-top{display:flex;justify-content:space-between;align-items:flex-start;gap:10px}
.brand{display:flex;gap:12px;align-items:center}
.brand-logo{width:38px;height:38px;object-fit:contain;filter:drop-shadow(0 2px 4px rgba(0,0,0,.25))}
...
.wave{
  position:absolute;
  left:0;right:0;
  bottom:-10px; /* baja la onda un poco */
...
/* Área de contenido */
.content-area{
  padding:0px 28px 34px; /* más padding superior */
  flex: 1;
}
.content-area.votar .step{
  margin-top: 20px;
}
...
.dni-wrap{display:flex;gap:10px;align-items:center;max-width:520px}
...
/* Tarjetas: estado “seleccionado” MUY visible */
.cards{
...
/* Glow y borde azulado cuando está seleccionado */
...
.actions{
  display:flex;
  gap:12px;
  margin-top:28px; /* más espacio arriba del botón */
  flex-wrap:wrap;
...
.dni-panel::before{
  /* borde superior suave como “glass” */
  content:"";
...
/* Compatibilidad: el viejo .dni-wrap ya no se usa aquí,
   pero se mantiene por si lo ocupas en otra parte. */
